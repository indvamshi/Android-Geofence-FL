	...
	private void handleMessage(Context context, Intent intent) {
		try {
			...
			NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(
					this).setSmallIcon(R.drawable.ic_launcher) // notification
																// icon
					.setContentTitle(Utils.notiMsg) // title for notification
					.setContentText("Message " + Utils.notiMsg) // message for
																// notification
			...
			database.InsertValue(Utils.notiLati, Utils.notiLongi,
					Utils.notiMsg, Utils.notiRadius, Utils.notiExp, "", "", "");
			...
			// Action for broadcast Intents containing various types of
			// geofencing errors
			...
			for (int i = 0; i < arrDetail.size(); i++) {
				...
				long expiration = arrDetail.get(i).getExpirationDuration()
						* GEOFENCE_EXPIRATION_IN_MILLISECONDS;// Long.parseLong(Utils.notiExp);
				int transition = Geofence.GEOFENCE_TRANSITION_ENTER;

				SimpleGeofence mGeofence = new SimpleGeofence(geofenceId, Lati,
						Longi, radius, expiration, transition);
			...
			System.out.println("geofenceId  "
					+ mGeofenceRequester.getInProgressFlag());
		} catch (Exception e) {
			...
			public void run() {
				...
				Thread t = new Thread(new Runnable() {
					...
					public void run() {
						...
						if (String.valueOf(IMEI).toString().trim()
								.equals("null")) {
							...
							IMEI = macAddr;
						} else if (String.valueOf(IMEI).toString().trim()
								.equals("")) {
							WifiManager wifiMan = (WifiManager) context
									.getSystemService(Context.WIFI_SERVICE);
							WifiInfo wifiInf = wifiMan.getConnectionInfo();
						...
						try {
							...
							//jsonObj.put("imei", IMEI.replace(":", ""));
							
							JSONArray array = new JSONArray();
							
							SharedPreferences sharedPreferences = getApplicationContext().getSharedPreferences
									("SharedPreferences", Context.MODE_PRIVATE);
							array.put(Double.parseDouble(sharedPreferences.getString("latitude", "invalid")));
							array.put(Double.parseDouble(sharedPreferences.getString("longitude", "invalid")));
							
							jsonObj.put("location", array);
							
							cmF.connectToServer(
									CommonFunctions.SIGNUPURL,
									jsonObj);
						} catch (Exception e) {
							e.printStackTrace();
							Log.e("Connecting To Server", "unable to send data to server, please check if the server is running or "
